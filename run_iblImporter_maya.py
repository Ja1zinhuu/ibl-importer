...
try:
    importlib.reload(sys.modules['dbQuery'])
    importlib.reload(sys.modules['iblImporter_mayaLogic'])
except:
    import iblImporter_mayaLogic
    import dbQuery
...
class iblImporter(QtWidgets.QDialog):
    ...
    def import_selected_items(self):
        if not self.user_selected_items:
            cmds.warning("No items selected for import")
            return
        ...
        for widget in self.user_selected_items:
            try:
                ...
                if item_data['itemType'] == "HDRI":
                    iblImporter_mayaLogic.create_light(item_data, file_path, intensity=widget.instensityQSpinBox.value(), 
                                        exposure=widget.exposureQSpinBox.value())
                elif item_data['itemType'] == "Lightmap":
                    iblImporter_mayaLogic.create_light(item_data,file_path, intensity=widget.instensityQSpinBox.value(), 
                                        exposure=widget.exposureQSpinBox.value())
                else:
                    cmds.warning(f"Unknown item type: {item_data['itemType']}")
                
                self.clear_selection_items()

                
            except Exception as e:
                cmds.warning(f"Error importing {item_data.get('name', 'unknown')}: {str(e)}")
                continue
    ...
    def add_item_to_sel_ui(self):
        ...
        if len(unproc_selected) + len(proc_selected) != 1:
            cmds.warning("Please select exactly one resolution (either from Unprocessed or Processed list)")
            return
